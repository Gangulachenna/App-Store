...
class AppStore extends Component {
  state = {
    activeTabItem: appsList[0].tabId,
    searchInput: '',
  }
  ...
  setActiveTabItem = tabId => {
    this.setState({activeTabItem: tabId})
  }

  getAppFilteredItems = eachApp => {
    const {activeTabItem} = this.state

    const filteredResult = eachApp.filter(eachApp.category === activeTabItem)
    return filteredResult
  }

  getSearchResult = () => {
    const {searchInput} = this.state
    const searchResult = appsList.filter(eachResult =>
  ...
  render() {
    ...
    const filteredResult = this.getAppFilteredItems(searchResult)

    return (
      <div className="app-container">
        <div className="app-strore">
          ...
          <ul className="apps-list">
            {filteredResult.map(eachAppItem => (
              <AppItem appDetails={eachAppItem} key={eachAppItem.appId} />
            ))}
          </ul>
        </div>
      </div>
